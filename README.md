...
* **Hardware/Emulation:** Python 3 (paho-mqtt) (pip install paho-mqtt)
* **Database:** H2 
...
* При запуске сервера должны вывестись сообщения о том, что база данных проинициализирована. Сделано для того, чтобы изначально на севере уже лежала датчик. Сейчас в него попадает датчик температуры.
